...
export class SuccessSfnTaskEventHandler
...
@EventHandler(TestSfnTaskEvent)
export class ErrorSfnTaskEventHandler
  implements IEventHandler<TestSfnTaskEvent>
{
  async execute(event: TestSfnTaskEvent): Promise<any> {
...
describe('TaskSfnEventHandler', () => {
  describe('execute', () => {
    ...
    beforeEach(async () => {
      ...
      eventBus.register([SuccessSfnTaskEventHandler])
    })

    it('should be defined with the success sfn handler', () => {
      expect(handler).toBeDefined()
    })
    it('should execute the task sfn event handler successfully', async () => {
      jest.spyOn(taskService, 'updateSubTaskStatus').mockResolvedValue()

      await handler.execute(event)

      expect(taskService.updateSubTaskStatus).toHaveBeenCalledTimes(2)
      expect(taskService.updateSubTaskStatus).toHaveBeenNthCalledWith(
        1,
        {
          pk: 'SFN_TASK#mbc',
          sk: 'test#01JKT45QNNBKJ9BEBHR3CXABYF#4',
  ...
  describe('execute', () => {
    ...
    beforeEach(async () => {
      ...
      taskService = module.get<TaskService>(TaskService)
      eventBus = module.get<EventBus>(EventBus)
      eventBus.register([ErrorSfnTaskEventHandler])
    })

    it('should be defined with the error sfn handler', () => {
      expect(handler).toBeDefined()
    })
    it('should execute the task sfn event handler and update the failed task status', async () => {
      jest.spyOn(taskService, 'updateSubTaskStatus').mockResolvedValue()
