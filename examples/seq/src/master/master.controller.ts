...
import { SequencesService } from '@mbc-cqrs-sererless/sequence'
import {
  Body,
  Controller,
  Get,
...
export class MasterController {
  ...
  async publishCommand(
    ...
    if (masterDto.version === 0) {
      const seqItem = await this.seqService.genNewSequence({
        tenantCode: 'common',
        typeCode: 'master',
      })
      masterDto.attributes.master['seq'] = seqItem.seq
    }
